...
export default function HomeDE() {
  ...
  return (
    <div className="min-h-screen bg-[var(--hawk-bg-primary)]">
        ...
        {mobileMenuOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="md:hidden bg-[var(--hawk-bg-primary)] border-t border-[var(--hawk-border)] p-4">
            <div className="flex flex-col gap-2">
              ...
              <div className="py-2 px-3"><span className="text-[var(--hawk-text-tertiary)] text-xs uppercase tracking-wider mb-2 block">Sprache</span><LanguageSelectorMobile currentLocale="de" /></div>
      ...
      <LifeTreeSection prefersReducedMotion={!!prefersReducedMotion} locale="de" />
      ...
      <section className="pt-32 pb-20 px-4">
        <div className="container mx-auto text-center max-w-4xl">
          ...
          <motion.div initial={prefersReducedMotion ? {} : { opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="https://github.com/tensorboy/hawkeye/releases" className="hawk-btn-accent inline-flex items-center justify-center gap-2"><Download className="w-5 h-5" />{t.hero.cta.download}</Link>
            <Link href="https://github.com/tensorboy/hawkeye" className="hawk-btn-ghost inline-flex items-center justify-center gap-2"><Github className="w-5 h-5" />{t.hero.cta.github}</Link>
          </motion.div>
      ...
      <section className="pb-20 px-4">
        <div className="container mx-auto max-w-4xl">
          <motion.div
            ...
            className="relative"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-[var(--hawk-accent)]/20 via-purple-500/20 to-cyan-500/20 blur-3xl -z-10" />
            <div className="bg-[var(--hawk-bg-secondary)] rounded-2xl border border-[var(--hawk-border)] shadow-2xl overflow-hidden">
              {/* Window Chrome */}
              <div className="flex items-center gap-2 px-4 py-3 bg-[var(--hawk-bg-tertiary)] border-b border-[var(--hawk-border)]">
      ...
      <section className="py-16 bg-[var(--hawk-bg-primary)] border-y border-[var(--hawk-border)]">
        <div className="container mx-auto max-w-6xl px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            >
              <div className="text-4xl md:text-5xl font-bold hawk-gradient-text mb-2">{t.stats.efficiency.value}</div>
              <div className="text-lg font-medium text-[var(--hawk-text-primary)] mb-1">{t.stats.efficiency.label}</div>
              <div className="text-sm text-[var(--hawk-text-secondary)]">{t.stats.efficiency.description}</div>
            </motion.div>
            <motion.div
              initial={prefersReducedMotion ? {} : { opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              ...
              className="text-center"
            >
              <div className="text-4xl md:text-5xl font-bold hawk-gradient-text mb-2">{t.stats.local.value}</div>
              <div className="text-lg font-medium text-[var(--hawk-text-primary)] mb-1">{t.stats.local.label}</div>
              <div className="text-sm text-[var(--hawk-text-secondary)]">{t.stats.local.description}</div>
            </motion.div>
            <motion.div
              initial={prefersReducedMotion ? {} : { opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              ...
              className="text-center"
            >
              <div className="text-4xl md:text-5xl font-bold hawk-gradient-text mb-2">{t.stats.platforms.value}</div>
              <div className="text-lg font-medium text-[var(--hawk-text-primary)] mb-1">{t.stats.platforms.label}</div>
              <div className="text-sm text-[var(--hawk-text-secondary)]">{t.stats.platforms.description}</div>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      {/* Features Section */}
      <section id="features" className="py-20 px-4 bg-[var(--hawk-bg-secondary)]">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {useCases.map((useCase, index) => (
              <motion.div key={index} initial={prefersReducedMotion ? {} : { opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }} className="hawk-card flex items-start gap-4">
                <div className="p-3 rounded-xl bg-[var(--hawk-bg-secondary)]"><useCase.icon className="w-6 h-6 text-[var(--hawk-accent)]" /></div>
      ...
      <section className="py-20 px-4 bg-[var(--hawk-bg-secondary)]">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="hawk-display-md text-[var(--hawk-text-primary)] mb-4">{t.testimonials.title}</h2>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {t.testimonials.items.map((testimonial, index) => (
              ...
              >
                ...
                <p className="text-[var(--hawk-text-secondary)] mb-6 italic">&ldquo;{testimonial.quote}&rdquo;</p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[var(--hawk-accent)] to-purple-600 flex items-center justify-center text-white font-bold">
                    {testimonial.avatar}
                  </div>
                  <div>
                    <div className="font-medium text-[var(--hawk-text-primary)]">{testimonial.author}</div>
      ...
      <section className="py-24 bg-[var(--hawk-bg-secondary)]">
        <div className="container mx-auto max-w-4xl px-4">
          <motion.div
            ...
            className="text-center"
          >
            <div className="inline-flex p-4 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 mb-6">
              <Shield className="w-10 h-10 text-white" />
            </div>
            <h2 className="hawk-display-md text-[var(--hawk-text-primary)] mb-6">{t.privacy.title}</h2>
            <div className="grid md:grid-cols-2 gap-4 max-w-2xl mx-auto text-left">
              <div className="flex items-center gap-3 p-4 rounded-xl bg-[var(--hawk-bg-primary)] border border-[var(--hawk-border)]">
          ...
          <div className="grid md:grid-cols-4 gap-8 mb-12">
            ...
            <div>
              ...
              <ul className="space-y-2 text-gray-400 text-sm">
                ...
                <li><Link href="#usecases" className="hover:text-white transition-colors">{t.footer.useCases}</Link></li>
                <li><Link href="#platforms" className="hover:text-white transition-colors">{t.footer.platforms}</Link></li>
            ...
            <div>
              ...
              <ul className="space-y-2 text-gray-400 text-sm">
                <li><a href="https://github.com/tensorboy/hawkeye#readme" className="hover:text-white transition-colors flex items-center gap-1">{t.footer.documentation}<ExternalLink className="w-3 h-3" /></a></li>
                <li><a href="https://github.com/tensorboy/hawkeye" className="hover:text-white transition-colors flex items-center gap-1">{t.footer.github}<ExternalLink className="w-3 h-3" /></a></li>
              </ul>
            </div>
            <div>
